...
static void do_execute (){
	if(DATA_BYTE==1){
		...
		set_of(op_dest->val,op_src->val,1,cpu.flags.bits.cf);
		set_sf(sum,1);
		set_zf(sum);
		set_pf(sum);
		set_af(op_dest->val,op_src->val,cpu.flags.bits.cf);	
		unsigned int result6=op_dest->val+op_src->val+cpu.flags.bits.cf;
	        unsigned int result7=((result6)<<23)>>31;
                cpu.flags.bits.cf=result7;
	}else if(DATA_BYTE==2){
		int16_t sum=(int16_t)op_dest->val+(int16_t)op_src->val;
		sum = sum + cpu.flags.bits.cf;
		OPERAND_W(op_dest,sum);
		set_of(op_dest->val,op_src->val,2,cpu.flags.bits.cf);
		set_sf(sum,2);
		set_zf(sum);
		set_pf(sum);
		set_af(op_dest->val,op_src->val,cpu.flags.bits.cf);	
		...
		sum = sum + cpu.flags.bits.cf;
		OPERAND_W(op_dest,sum);
		set_of(op_dest->val,op_src->val,4,cpu.flags.bits.cf);
		set_sf(sum,4);
		set_zf(sum);
		set_pf(sum);
		set_af(op_dest->val,op_src->val,cpu.flags.bits.cf);	
